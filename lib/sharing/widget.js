...
import { parseChartAddons, shareChartAddons } from './addons.js';
import { parseDrawings, shareDrawings } from './drawings.js';
import { newMetricAlias, shareAxesMetrics, shareColors, shareMetricSettings, shareCombinedMetrics, } from './index.js';
import { shareMetrics } from './metrics.js';
import { parseAxesMetrics, parseCombinedMetrics, parseIndicators, parseMergedMetrics, parseMetricGraphValue, parseMetrics, parsePinnedAxesMetrics, } from './parse.js';
function nullify(value) {
    if (Array.isArray(value) && value.length === 0)
        return undefined;
    return value || undefined;
}
function nullifyMetricSettings(sharedSettings) {
    let hasValues = false;
    const result = {};
...
function shareSignalMetrics(signalMetrics, accessor) {
...
export function parseMetricSettings(settings, metrics) {
    ...
    Object.keys(settings || {}).forEach((alias) => {
        ...
        if (metric)
            result[metric.key] = settings[alias];
    });
    return result;
}
...
export function parseWidget(shared, ctx) {
    ...
    Object.assign(parsed, ctx === null || ctx === void 0 ? void 0 : ctx.parseSubwidgets(wcsb));
