...
canvas.ontouchend = canvas.onmouseleave = () => {
    ...
    document.body.classList.remove('hovering-dOva2l');
};
canvas.ontouchstart =
    canvas.ontouchmove =
        canvas.onmousemove =
            handlePointEvent(chart, (point, e) => {
                ...
                if (isMouveEvent === false) {
                    if (document.body.classList.contains('hovering-dOva2l') === false) {
                        document.body.classList.add('hovering-dOva2l');
                    }
                }
