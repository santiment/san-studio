...
import { getDrawingUpdater } from './drawings/index.js';
import { getLineDragData, lineDragModifier } from './drawings/line.js';
import { getEmojiDragData, emojiDragModifier } from './drawings/emoji/index.js';
import { noteDragModifier, handleNoteDoubleClick } from './drawings/note.js';
import { horizontalRayDragModifier } from './drawings/horizontalRay.js';
import { verticalRayDragModifier } from './drawings/verticalRay.js';
...
export function newDoubleClickHandler(chart, onDrawingModified) {
    ...
    return function onDoubleClick() {
        ...
        if (!selected)
            return;
        (_a = DrawingDoubleClickHandler[selected.type]) === null || _a === void 0 ? void 0 : _a.call(DrawingDoubleClickHandler, drawer, selected, onDrawingModified);
    };
}
export function newDrawingSelectHandler(chart, controller) {
    ...
    return function onMouseDown(e) {
        ...
        const onDrawingDrag = newDrawingDragHandler(chart, hovered, getEventCoordinates(e), dpr, wasDragged);
...
function newDrawingDragHandler(chart, drawing, [startX, startY], dpr, wasDragged) {
    ...
    if (!(updater && dragModifier))
        return;
    const { ctx, updateRelativeByAbsoluteCoordinates } = drawer;
    const dragData = getDragData === null || getDragData === void 0 ? void 0 : getDragData(ctx, drawing, startX * dpr, startY * dpr);
...
export const newDrawingDeleteHandler = (drawer) => ({ key }) => {
    if (key !== 'Backspace' || !drawer.selected)
        return;
