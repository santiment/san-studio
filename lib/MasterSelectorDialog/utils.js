...
export function addRecent(_a) {
    var { priceUsd: _ } = _a, item = __rest(_a, ["priceUsd"]);
    const { key, rawKey = key } = item;
    const recents = getRecents()
        .filter((recent) => recent.rawKey !== rawKey)
        .slice(0, 4);
    saveJson(RECENT_MASTER_SELECTIONS, [Object.assign(Object.assign({}, item), { rawKey, key: RECENT_KEY + rawKey }), ...recents]);
}
export function getRecentAssetMap(recents) {
    const map = {};
    recents.forEach((recent) => {
        if (recent.slug)
            map[recent.slug] = recent;
    });
    return map;
}
const hashMetrics = (metrics) => metrics
    .map(({ key }) => key)
    .sort()
    .toString();
...
export function replaceDefaultMetrics(item, widgets) {
    ...
    if (hash !== PROJECT_HASH && hash !== ADDRESS_HASH)
        return;
