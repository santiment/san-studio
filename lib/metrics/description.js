...
export function prepareDescription(description, ticker) {
    return description.replace(TICKER_REGEX, ticker);
}
export const Description = {
    [Metric.social_volume_total.key]: 'Shows the amount of mentions of the coin on 1000+ crypto social media channels, including Telegram groups, crypto subreddits, discord groups, private traders chats and more.',
    [Metric.social_volume_ai_total.key]: 'This metric computed using Natural Language Processing (NLP) technique such as Named Entity Recognition (NER). Our trained NER model can process large amount of social data and extract from it mentions about crypto assets.',
    ...
    [Metric.realized_cap_hodl_waves_0d_to_1d.key]: 'Shows the percentage of [Project Ticker] realized cap for a given timebound interval in total realized cap value. Realized cap HODL waves is calculated by dividing the [Project Ticker] realized cap in given period by [Project Ticker] total realized cap',
    ...
    [Metric.nvt.key]: 'Shows the relationship between the [Project Ticker] market cap and the amount of [Project Ticker] transacted on-chain daily. High NVT levels indicate and overvalued asset, and vice versa. This version of NVT is calculated by dividing the [Project Ticker] market cap by its daily circulation, or the amount of <em>unique</em> [Project Ticker] exchanged on the network daily',
    ...
    addresses_number_distribution: 'Breaks down the number of addresses based on the amount of [Project Ticker] they hold.',
    labeled_addresses_number_distribution: 'Breaks down the number of addresses based on the amount of [Project Ticker] they hold and labeled by owners. Labels are available only for Ethereum/ERC-20 assets',
    addresses_balance_distribution: 'Represents the sum of all the tokens of all the addresses which hold the amount of tokens from an interval.',
...
For each coin that moves on-chain, NPL takes the price at which it was last moved and assumes this to be its acquisition price. Once it changes addresses again, NPL assumes the coin was sold.`,
    ...
    [(_a = SelectorNode === null || SelectorNode === void 0 ? void 0 : SelectorNode.SPENT_COIN_COST) === null || _a === void 0 ? void 0 : _a.key]: 'It shows at what price the tokens that were transacted today were last moved. More info <a target="_blank" rel="noopener noreferrer" href="https://academy.santiment.net/metrics/spent-coins-age-band/ ">here</a>',
