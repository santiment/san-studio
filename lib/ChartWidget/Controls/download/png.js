...
import { getPadding } from './../../../Chart/Axes/utils.js';
import { PLOT_ID } from './../../../Chart/Drawer/drawer.js';
import { drawDrawings } from './../../../Chart/Drawer/drawings/index.js';
...
function getLabel(metric, ticker) {
    ...
    if (project)
        return label;
    return ((_a = metric.getLabel) === null || _a === void 0 ? void 0 : _a.call(metric, ticker)) || `${label} (${ticker})`;
}
export function drawLegend(pngChart, metrics, textColor, ticker, font = LEGEND_FONT) {
    ...
    const textWidth = metrics.reduce((acc, metric) => acc +
        ...
        pngCtx.measureText(getLabel(metric, ticker)).width, 0) +
...
function setupPngDrawings(chart, pngChart, plotters) {
    if (chart.drawer.isHidden)
        return plotters.delete(PLOT_ID);
