...
import { MetricType, newKey } from './../../lib/metrics/utils'
import { getType } from './../../lib/metrics/_onchain/holderDistributions'
...
function fetch(variables, metric, cachePolicy) {
  ...
  const queries = baseMetrics.map(({ key, queryKey = key }) =>
    queryMetric(
      Object.assign(Object.assign({}, variables), { key, metric: queryKey }),
      undefined,
      cachePolicy,
    ).then(dataAccessor),
  )
