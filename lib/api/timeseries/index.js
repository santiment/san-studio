...
export function getTimeseries(metrics, variables, onData, onError, MetricSettings = {}, cachePolicy, requestOptions) {
    ...
    for (let i = 0; i < metrics.length; i++) {
        ...
        // prettier-ignore
        const { interval, slug, from, to, transform, owner, labels, label, holdersCount, label_fqn, address } = Object.assign({}, variables, metricSettings);
        // prettier-ignore
        const vars = Object.assign({ key, metric: queryKey, from, to, interval, transform, owner, labels, label, holdersCount, label_fqn, [selector]: slug, address }, reqMeta);
